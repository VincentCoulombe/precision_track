...
- **There are eleven tools in the `tools` directory**
  ...
  - `pretrain_action_recognition.py` — **Self-supervised (masked autoencoder) pretraining** of a **MART** model from **raw video only**, with no action labels.
...
## 2b) pretrain_action_recognition.py

- **Purpose:** **Self-supervised pretraining** of a **MART** model. MART masks a fraction of its own sequence embeddings and learns to reconstruct them (a masked-autoencoder objective), so it needs **no action labels** — only raw video. Use it to warm-start a supervised `train_action_recognition.py` run when you have plenty of footage but few annotations.

- **Inputs:**:
  - `--config`. Path to the pretraining config file. Default to `../configs/tasks/training_action_recognition_unsup.py`.
  - Resuming an interrupted run is controlled by `training.resume` in your `user_configs.yaml`, as it is for the other training tools.

- **Requirements:** a trained **detector** in your `<deploying_directory>` (the sequences are generated by tracking the raw videos) and `with_action_recognition: true` in your `user_configs.yaml`. Unlike `train_action_recognition.py`, it needs **no `action_recognition_data_root`** and **no existing MART checkpoint** — MART is trained from scratch.

- **Configuration:** the videos to pretrain on are the `train_sequences` / `val_sequences` lists of the config. Validation reports `MSEMetric` (the reconstruction error — lower is better), and the best checkpoint is selected on `MSEMetric/mean`.

- **Outputs:** The training log and the best/last checkpoints will be saved in the `precision_track/work_dir/training_runs/<dataset_name>` directory. The resulting weights are an **initialisation**, not a deployable model — the classification head is still untrained, so there is no `--deploy` step. Point `training_checkpoint` at the produced checkpoint and run `train_action_recognition.py` to fine-tune it on labelled actions.

- **Note:** sequence generation runs the detector over every frame, which is slow on CPU (in the order of seconds per frame). A CUDA-accelerated machine is strongly recommended.

- **Examples**

  ```bash
  python pretrain_action_recognition.py
  python pretrain_action_recognition.py --config ../configs/tasks/training_action_recognition_unsup.py
  ```
