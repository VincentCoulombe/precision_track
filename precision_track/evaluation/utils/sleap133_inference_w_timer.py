...
from sleap.nn.data.pipelines import InstanceCentroidFinder, KerasModelPredictor, LabelsReader, Normalizer, Pipeline, Prefetcher, Provider, Resizer, VideoReader
...
class RateColumn(rich.progress.ProgressColumn):
    ...
    def render(self, task) -> rich.progress.Text:
        """Show progress rate."""
        speed = task.speed
        if speed is None:
...
class Predictor(ABC):
    ...
    def from_model_paths(
        ...
        """Create the appropriate `Predictor` subclass from a list of model
        paths.
        ...
        if isinstance(model_paths, str):
            ...
            if "movenet" in model_paths:
                ...
                predictor = MoveNetPredictor.from_trained_models(model_name=model_name, peak_threshold=peak_threshold)
        ...
        model_types = [cfg.model.heads.which_oneof_attrib_name() for cfg in model_configs]
        ...
        if "single_instance" in model_types:
            ...
            raise ValueError("Could not create predictor from model paths:" + "\n".join(model_paths))
    ...
    def make_pipeline(self, data_provider: Optional[Provider] = None) -> Pipeline:
        ...
        pipeline += sleap.nn.data.pipelines.Batcher(batch_size=self.batch_size, drop_remainder=False, unrag=False)
    ...
    def _predict_generator(self, data_provider: Provider) -> Iterator[Dict[str, np.ndarray]]:
        ...
        def process_batch(ex):
            ...
            ex["instance_peaks"] /= np.expand_dims(np.expand_dims(ex["scale"], axis=1), axis=1)

            return ex

        # Loop over data batches with optional progress reporting.
        self.prediction_delays = []
        if self.verbosity == "rich":
            with rich.progress.Progress(
                ...
                for ex in self.pipeline.make_dataset():
                    t0 = time()
                    ex = process_batch(ex)
                    self.prediction_delays.append((time() - t0) / ex["image"].shape[0])
            ...
            for ex in self.pipeline.make_dataset():
                ...
                # Report.
                elapsed_since_last_report = time() - last_report
                self.prediction_delays.append(elapsed_since_last_report)
                if elapsed_since_last_report > self.report_period:
                    print(
                        json.dumps(
                            {
    ...
    def predict(self, data: Union[Provider, sleap.Labels, sleap.Video], make_labels: bool = True) -> Union[List[Dict[str, np.ndarray]], sleap.Labels]:
        ...
        if make_labels:
            ...
            return (
                sleap.Labels(self._make_labeled_frames_from_generator(generator, data)),
                self.prediction_delays,
            )
        else:
            # Just return the raw results.
            return list(generator), np.array(self.prediction_delays)
    ...
    def export_model(
        ...
        self.inference_model.export_model(save_path, signatures, save_traces, model_name, tensors, unrag_outputs)
...
class VisualPredictor(Predictor):
    ...
    def make_pipeline(self):
        ...
        pipeline += Resizer.from_config(self.config.data.preprocessing, keep_full_image=False, points_key=None)
            ...
            try:
                ...
                if progress:
                    if (i and i % 1000 == 0) or done:
                        ...
                        logger.info(f"Finished {i} examples in {elapsed_time:.2f} seconds " "(inference + postprocessing)")
                        if elapsed_time:
...
class CentroidCropGroundTruth(tf.keras.layers.Layer):
    ...
    def call(self, example_gt: Dict[str, tf.Tensor]) -> Dict[str, tf.Tensor]:
        ...
        crops = tf.reshape(crops, [n_peaks, self.crop_size, self.crop_size, img_channels])

        # Group crops by sample.
        samples = tf.shape(full_imgs, out_type=tf.int64)[0]

        crops = tf.RaggedTensor.from_value_rowids(crops, crop_sample_inds, nrows=samples)
        crop_offsets = tf.RaggedTensor.from_value_rowids(crop_offsets, crop_sample_inds, nrows=samples)
        centroid_vals = tf.RaggedTensor.from_value_rowids(centroid_vals, crop_sample_inds, nrows=samples)
...
class FindInstancePeaksGroundTruth(tf.keras.layers.Layer):
    ...
    def call(self, example_gt: Dict[str, tf.Tensor], crop_output: Dict[str, tf.Tensor]) -> Dict[str, tf.Tensor]:
        ...
        # each sample.
        a = tf.expand_dims(example_gt["instances"].with_row_splits_dtype(tf.int64), axis=1)  # (batch_size, 1, n_insts, n_nodes, 2)
        a = a.to_tensor(default_value=tf.cast(np.NaN, tf.float32))
        b = tf.expand_dims(tf.expand_dims(crop_output["centroids"], axis=2), axis=2).with_row_splits_dtype(tf.int64)  # (batch_size, n_centroids, 1, 1, 2)
        ...
        dists = dists.to_tensor(tf.cast(np.NaN, tf.float32))  # (batch_size, n_centroids, n_insts)
        ...
        instance_peak_vals = tf.gather(tf.ones_like(instance_peaks, dtype=tf.float32), 0, axis=-1)  # (batch_size, n_centroids, n_nodes)
...
class InferenceModel(tf.keras.Model):
    ...
    def export_model(
        ...
        full_model = tf.function(lambda x: (sleap.nn.data.utils.unrag_example(model(x), numpy=False) if unrag_outputs else model(x)))
        ...
        with (Path(save_path) / "info.json").open("w") as fp:
            json.dump(info, fp, indent=4, sort_keys=True, separators=(",", ": "), default=str)
...
def get_model_output_stride(model: tf.keras.Model, input_ind: int = 0, output_ind: int = -1) -> int:
    ...
    if size_in % size_out != 0:
        warnings.warn(
            f"Model input of shape {model.inputs[input_ind].shape} does not divide " f"evenly with output of shape {model.outputs[output_ind].shape}."
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.confmaps_ind is None:
            self.confmaps_ind = find_head(self.keras_model, "SingleInstanceConfmapsHead")

        if self.confmaps_ind is None:
            raise ValueError("Index of the confidence maps output tensor must be specified if not " "named 'SingleInstanceConfmapsHead'.")
...
class SingleInstancePredictor(Predictor):
    ...
    def _initialize_inference_model(self):
        """Initialize the inference model from the trained model and
        configuration."""
    ...
    def from_trained_models(
        ...
        confmap_model.keras_model = tf.keras.models.load_model(confmap_keras_model_path, compile=False)
        if resize_input_layer:
            confmap_model.keras_model = reset_input_layer(keras_model=confmap_model.keras_model, new_shape=None)
        obj = cls(
    ...
    def _make_labeled_frames_from_generator(self, generator: Iterator[Dict[str, np.ndarray]], data_provider: Provider) -> List[sleap.LabeledFrame]:
        """Create labeled frames from a generator that yields inference
        results.
...
class CentroidCrop(InferenceLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.confmaps_ind is None:
            raise ValueError("Index of the confidence maps output tensor must be specified if not " "named 'CentroidConfmapsHead'.")
        ...
        if output_stride is None:
            # Attempt to automatically infer the output stride.
            output_stride = get_model_output_stride(self.keras_model, output_ind=self.confmaps_ind)
        self.output_stride = output_stride
        self.peak_threshold = peak_threshold
        self.refinement = refinement
        self.integral_patch_size = integral_patch_size
    ...
    def call(self, inputs):
        ...
        if n_peaks > 0:
            if self.max_instances is not None:
                centroid_points = tf.RaggedTensor.from_value_rowids(centroid_points, crop_sample_inds, nrows=samples)
                centroid_vals = tf.RaggedTensor.from_value_rowids(centroid_vals, crop_sample_inds, nrows=samples)
                ...
                for sample in range(samples):
                    ...
                    if self.max_instances < n_centroids:
                        ...
                        _centroid_vals = _centroid_vals.write(sample, tf.gather(centroid_vals[sample], top_inds))

                        _centroid_points = _centroid_points.write(sample, tf.gather(centroid_points[sample], top_inds))

                        _row_ids = _row_ids.write(sample, tf.fill([len(top_inds)], sample))
                    else:
                        _centroid_vals = _centroid_vals.write(sample, centroid_vals[sample])
                        _centroid_points = _centroid_points.write(sample, centroid_points[sample])
                        _row_ids = _row_ids.write(sample, tf.fill([n_centroids], sample))
            ...
            # Reshape to (n_peaks, crop_height, crop_width, channels)
            crops = tf.reshape(crops, [n_peaks, self.crop_size, self.crop_size, full_imgs.shape[3]])
        ...
        centroids = tf.RaggedTensor.from_value_rowids(centroid_points, crop_sample_inds, nrows=samples)
        centroid_vals = tf.RaggedTensor.from_value_rowids(centroid_vals, crop_sample_inds, nrows=samples)

        outputs = dict(centroids=centroids, centroid_vals=centroid_vals)
        if self.return_confmaps:
            # Return confidence maps with outputs.
            cms = tf.RaggedTensor.from_value_rowids(cms, crop_sample_inds, nrows=samples)
...
class FindInstancePeaks(InferenceLayer):
    """Keras layer that predicts instance peaks from images using a trained
    model.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(keras_model=keras_model, input_scale=input_scale, pad_to_stride=1, **kwargs)
        self.peak_threshold = peak_threshold
        self.refinement = refinement
        self.integral_patch_size = integral_patch_size
        self.return_confmaps = return_confmaps
        ...
        if self.confmaps_ind is None:
            self.confmaps_ind = find_head(self.keras_model, "CenteredInstanceConfmapsHead")

        if self.confmaps_ind is None:
            raise ValueError("Index of the confidence maps output tensor must be specified if not " "named 'CenteredInstanceConfmapsHead'.")
    ...
    def call(self, inputs: Union[Dict[str, tf.Tensor], tf.Tensor]) -> Dict[str, tf.Tensor]:
        ...
        peaks = tf.RaggedTensor.from_value_rowids(peak_points, crop_sample_inds, nrows=samples)
        peak_vals = tf.RaggedTensor.from_value_rowids(peak_vals, crop_sample_inds, nrows=samples)
...
class CentroidInferenceModel(InferenceModel):
    ...
    def call(self, example: Union[Dict[str, tf.Tensor], tf.Tensor]) -> Dict[str, tf.Tensor]:
        """Predict instances for one batch of images.
    ...
    def call(self, example: Union[Dict[str, tf.Tensor], tf.Tensor]) -> Dict[str, tf.Tensor]:
        ...
        if isinstance(self.instance_peaks, FindInstancePeaksGroundTruth):
            if "instances" in example:
                ...
                raise ValueError("Ground truth data was not detected... " "Please load both models when predicting on non-ground-truth data.")
        else:
...
class TopDownPredictor(Predictor):
    ...
    def _initialize_inference_model(self):
        """Initialize the inference model from the trained models and
        configuration."""
        use_gt_centroid = self.centroid_config is None
        use_gt_confmap = self.confmap_config is None

        if use_gt_centroid:
            centroid_crop_layer = CentroidCropGroundTruth(crop_size=self.confmap_config.data.instance_cropping.crop_size)
        else:
            if use_gt_confmap:
                crop_size = 1
            else:
        ...
        self.inference_model = TopDownInferenceModel(centroid_crop=centroid_crop_layer, instance_peaks=instance_peaks_layer)

    @property
    def data_config(self) -> DataConfig:
        return self.centroid_config.data if self.centroid_config else self.confmap_config.data
    ...
    def from_trained_models(
        ...
        if centroid_model_path is None and confmap_model_path is None:
            raise ValueError("Either the centroid or topdown confidence map model must be provided.")
        ...
        if centroid_model_path is not None:
            ...
            centroid_model.keras_model = tf.keras.models.load_model(centroid_keras_model_path, compile=False)
            if resize_input_layer:
                # Reset input layer dimensions to be more flexible
                centroid_model.keras_model = reset_input_layer(keras_model=centroid_model.keras_model, new_shape=None)
        else:
    ...
    def _make_labeled_frames_from_generator(self, generator: Iterator[Dict[str, np.ndarray]], data_provider: Provider) -> List[sleap.LabeledFrame]:
        ...
        def _object_builder():
            while True:
                ...
                if "n_valid" in ex:
                    ex["instance_peaks"] = [x[:n] for x, n in zip(ex["instance_peaks"], ex["n_valid"])]
                    ex["instance_peak_vals"] = [x[:n] for x, n in zip(ex["instance_peak_vals"], ex["n_valid"])]
                    ex["centroids"] = [x[:n] for x, n in zip(ex["centroids"], ex["n_valid"])]
                    ex["centroid_vals"] = [x[:n] for x, n in zip(ex["centroid_vals"], ex["n_valid"])]
    ...
    def export_model(
        ...
        if self.centroid_config is not None:
            self.centroid_config.save_json(os.path.join(save_path, "centroid_config.json"))
...
class BottomUpInferenceLayer(InferenceLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.confmaps_ind is None:
            raise ValueError("Index of the confidence maps output tensor must be specified if not " "named 'MultiInstanceConfmapsHead'.")

        if self.pafs_ind is None:
            self.pafs_ind = find_head(self.keras_model, "PartAffinityFieldsHead")

        if self.pafs_ind is None:
            raise ValueError("Index of the part affinity fields output tensor must be specified if " "not named 'PartAffinityFieldsHead'.")
    ...
    def find_peaks(self, cms, offsets):
        ...
        peaks = tf.RaggedTensor.from_value_rowids(peaks, peak_sample_inds, nrows=n_samples)
        peak_vals = tf.RaggedTensor.from_value_rowids(peak_vals, peak_sample_inds, nrows=n_samples)
        peak_channel_inds = tf.RaggedTensor.from_value_rowids(peak_channel_inds, peak_sample_inds, nrows=n_samples)
...
class BottomUpPredictor(Predictor):
    ...
    def from_trained_models(
        ...
        bottomup_model.keras_model = tf.keras.models.load_model(bottomup_keras_model_path, compile=False)
        if resize_input_layer:
            bottomup_model.keras_model = reset_input_layer(keras_model=bottomup_model.keras_model, new_shape=None)
        obj = cls(
        ...
        def _object_builder():
            while True:
                ...
                if "n_valid" in ex:
                    ...
                    ex["instance_peaks"] = [x[:n] for x, n in zip(ex["instance_peaks"], ex["n_valid"])]
                    ex["instance_peak_vals"] = [x[:n] for x, n in zip(ex["instance_peak_vals"], ex["n_valid"])]
                    ex["instance_scores"] = [x[:n] for x, n in zip(ex["instance_scores"], ex["n_valid"])]
                ...
                for image, video_ind, frame_ind, points, confidences, scores in zip(
                    ...
                    if self.max_instances is not None:
                        # Filter by score.
                        predicted_instances = sorted(predicted_instances, key=lambda x: x.score, reverse=True)
                        predicted_instances = predicted_instances[: min(self.max_instances, len(predicted_instances))]
...
class BottomUpMultiClassInferenceLayer(InferenceLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.class_maps_ind is None:
            raise ValueError("Index of the part affinity fields output tensor must be specified if " "not named 'ClassMapsHead'.")
        ...
        if class_maps_output_stride is None:
            # Attempt to automatically infer the output stride.
            class_maps_output_stride = get_model_output_stride(self.keras_model, output_ind=self.class_maps_ind)
    ...
    def find_peaks(self, cms, offsets):
        ...
        if self.offsets_ind is None:
            ...
            (
                ...
                peak_sample_inds,
                peak_channel_inds,
            ) = sleap.nn.peak_finding.find_local_peaks_with_offsets(cms, offsets, threshold=self.peak_threshold)
    ...
    def call(self, data):
        ...
        peaks, peak_vals, peak_sample_inds, peak_channel_inds = self.find_peaks(cms, offsets)
...
class BottomUpMultiClassPredictor(Predictor):
    ...
    def from_trained_models(
        ...
        model.keras_model = tf.keras.models.load_model(keras_model_path, compile=False)
        if resize_input_layer:
            model.keras_model = reset_input_layer(keras_model=model.keras_model, new_shape=None)
        obj = cls(
            config=config,
            model=model,
            peak_threshold=peak_threshold,
        ...
        def _object_builder():
            while True:
                ...
                for image, video_ind, frame_ind, points, confidences, scores in zip(
                    ...
                    # Loop over instances.
                    predicted_instances = []
                    for i, (pts, confs, score) in enumerate(zip(points, confidences, scores)):
                        if np.isnan(pts).all():
                            continue
                        track = None
...
class TopDownMultiClassFindPeaks(InferenceLayer):
    """Keras layer that predicts and classifies peaks from images using a
    trained model.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.class_vectors_ind is None:
            raise ValueError("Index of the classifications output tensor must be specified if not " "named 'ClassVectorsHead'.")
    ...
    def call(self, inputs: Union[Dict[str, tf.Tensor], tf.Tensor]) -> Dict[str, tf.Tensor]:
        ...
        if self.offsets_ind is None:
            ...
            (
                peak_points,
                peak_vals,
            ) = sleap.nn.peak_finding.find_global_peaks_with_offsets(cms, offsets, threshold=self.peak_threshold)
        ...
        if self.optimal_grouping:
            ...
            (
                ...
                point_vals,
                class_probs,
            ) = sleap.nn.identity.classify_peaks_from_vectors(peak_points, peak_vals, peak_class_probs, crop_sample_inds, samples)
...
class TopDownMultiClassInferenceModel(InferenceModel):
    ...
    def export_model(
        ...
        super().export_model(save_path, signatures, save_traces, model_name, tensors, unrag_outputs)
...
class TopDownMultiClassPredictor(Predictor):
    ...
    def _initialize_inference_model(self):
        ...
        use_gt_confmap = self.confmap_config is None
        if use_gt_confmap:
            raise ValueError("Both a centroid and a confidence map model must be provided to " "initialize a TopDownMultiClassPredictor.")
        ...
        self.inference_model = TopDownMultiClassInferenceModel(centroid_crop=centroid_crop_layer, instance_peaks=instance_peaks_layer)
    ...
    def _make_labeled_frames_from_generator(self, generator: Iterator[Dict[str, np.ndarray]], data_provider: Provider) -> List[sleap.LabeledFrame]:
        ...
        if tracks is None:
            if hasattr(data_provider, "tracks"):
                tracks = data_provider.tracks
            elif self.confmap_config.model.heads.multi_class_topdown.class_vectors.classes is not None:
                names = self.confmap_config.model.heads.multi_class_topdown.class_vectors.classes
                tracks = [sleap.Track(name=n, spawned_on=0) for n in names]
...
def make_model_movenet(model_name: str) -> tf.keras.Model:
    ...
    x = tf.keras.layers.Lambda(lambda x: tf.cast(x, dtype=tf.int32), name="cast_to_int32")(x_in)
...
class MoveNetPredictor(Predictor):
    ...
    @classmethod
    def from_trained_models(cls, model_name: Text, peak_threshold: float = 0.2) -> "MoveNetPredictor":
        """Create the predictor from a saved model.
...
def _make_export_cli_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        dest="models",
        action="append",
        help=("Path to trained model directory (with training_config.json). " "Multiple models can be specified, each preceded by --model."),
    )
    parser.add_argument(
        "-e",
        "--export_path",
        ...
        help=("Path to output directory where the frozen model will be exported to. " "Defaults to a folder named 'exported_model'."),
    )
    parser.add_argument(
        "-u",
        "--unrag",
        action="store_true",
        default=True,
        help=("Convert ragged tensors into regular tensors with NaN padding. " "Defaults to True."),
    )
    parser.add_argument(
        "-n",
        "--max_instances",
...
def _make_cli_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=("The output filename to use for the predicted data. If not provided, " "defaults to '[data_path].predictions.slp'."),
    )
    parser.add_argument(
        "--no-empty-frames",
        action="store_true",
        default=False,
        help=("Clear any empty frames that did not have any detected instances before " "saving to output."),
    )
    parser.add_argument(
        "--verbosity",
        type=str,
    ...
    parser.add_argument("--video.dataset", type=str, default=None, help="The dataset for HDF5 videos.")
    parser.add_argument(
        "--video.input_format",
        type=str,
    ...
    device_group.add_argument(
        ...
        type=str,
        default="auto",
        help=("Run training on the i-th GPU on the system. If 'auto', run on the GPU with" " the highest percentage of available memory."),
    )
    parser.add_argument(
...
def _make_provider_from_cli(args: argparse.Namespace) -> Tuple[Provider, str]:
    ...
    if data_path is None or data_path == "":
        raise ValueError("You must specify a path to a video or a labels dataset. " "Run 'sleap-track -h' to see full command documentation.")
    ...
    if data_path.endswith(".slp"):
        ...
        provider = VideoReader.from_filepath(filename=data_path, example_indices=frame_list(args.frames), **video_kwargs)
...
def _make_predictor_from_cli(args: argparse.Namespace) -> Predictor:
    ...
    if args.models is not None:
        ...
        if isinstance(predictor, BottomUpPredictor):
            predictor.inference_model.bottomup_layer.paf_scorer.max_edge_length_ratio = args.max_edge_length_ratio
            predictor.inference_model.bottomup_layer.paf_scorer.dist_penalty_weight = args.dist_penalty_weight
    else:
        ...
        if args.first_gpu:
            ...
            if args.gpu == "auto":
                ...
                if len(free_gpu_memory) > 0:
                    ...
                    logger.info(f"Auto-selected GPU {gpu_ind} with {mem} MiB of free memory.")
                else:
                    logger.info("Failed to query GPU memory from nvidia-smi. Defaulting to " "first GPU.")
                    gpu_ind = 0
            else:
