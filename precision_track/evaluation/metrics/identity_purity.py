...
from precision_track.evaluation.utils.mot import evaluate_identity_purity
from precision_track.registry import METRICS


@METRICS.register_module()
class IdentityPurityMetrics(BaseMetric):
    """Frame-weighted GT-side identity purity.

    For each GT identity g, let f_gp be the frames where g and predicted id p
    are Hungarian-matched, and f_g the frames where g exists. Purity is
    sum_g max_p f_gp / sum_g f_g — the fraction of GT-present frames carrying
    the dominant predicted id for that GT. Unlike IDF1 it does not enforce a
    global one-to-one mapping, so a tracker that briefly swaps and recovers
    only loses the frames inside the swap window.
    """

    default_prefix = "IdentityPurity"
    metrics_agg = dict(
        purity_gt="mean",
        num_gt_frames="sum",
    )
    ...
    def __init__(
        ...
        if output_file is not None:
            ...
            self.logger.info(f"The identity-purity results will be saved at {os.path.abspath(self.output_file)}.")

    def process(self, data_batch: Any, data_samples: Any) -> None:
        for pred, gt in zip(data_batch, data_samples):
            evaluation_results = evaluate_identity_purity(
                pred,
                gt,
                self.metainfo,
                save_path=None,
