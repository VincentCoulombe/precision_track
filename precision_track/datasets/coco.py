...
from precision_track.registry import DATASETS
...
class COCODataset(BaseDataset):
    ...
    def __init__(
        ...
        if bbox_file:
            if self.data_mode != "topdown":
                raise ValueError(f"{self.__class__.__name__} is set to {self.data_mode}: " 'mode, while "bbox_file" is only ' "supported in topdown mode.")

            if not test_mode:
                raise ValueError(f"{self.__class__.__name__} has `test_mode==False` " 'while "bbox_file" is only ' "supported when `test_mode==True`.")
    ...
    def _load_annotations(self) -> Tuple[List[dict], List[dict]]:
        ...
        assert instance_list, f"The {self.ann_file} annotation file does not contain COCO-formatted annotations."
        assert image_list, f"The {self.ann_file} annotation file does not contain COCO-formatted images."
    ...
    def parse_data_info(self, raw_data_info: dict) -> Optional[dict]:
        ...
        metadata_num_keypoints = self.metainfo["num_keypoints"]
        if "keypoints" not in ann:
            assert metadata_num_keypoints > 0, "Need at least 1 kpts to not break the data transformation pipelines."
            ann["keypoints"] = [0, 0, 0] * metadata_num_keypoints
    ...
    def _load_detection_results(self) -> List[dict]:
        ...
        for det in det_results:
            ...
            bbox = reformat(bbox_xywh, "xywh", "xyxy")
    ...
    def filter_data(self) -> List[dict]:
        ...
        if "bbox_score_thr" in self.filter_cfg:
            ...
            if self.data_mode != "topdown":
                raise ValueError(f"{self.__class__.__name__} is set to {self.data_mode} " 'mode, while "bbox_score_thr" is only supported in ' "topdown mode.")
