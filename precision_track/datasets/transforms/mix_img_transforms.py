...
from .formatting import flip_bbox, flip_keypoints, imresize
...
class Mosaic(MixImageTransform):
    ...
    def apply_mix(self, results: dict) -> dict:
        ...
        bboxes = clip(bboxes, "xyxy", 2 * self.img_scale[0], 2 * self.img_scale[1])
        kpts, kpts_vis = clip(kpts, "keypoints_vis", max_width=2 * self.img_scale[0], max_height=2 * self.img_scale[1], keypoints_visible=kpts_vis)
    ...
    def _create_mosaic_image(self, results, mixed_data_list):
        ...
        for loc, data in zip(locs, (results, *mixed_data_list)):
            ...
            img = imresize(img, (int(w * scale_ratio), int(h * scale_ratio)))
...
class YOLOXMixUp(MixImageTransform):
    ...
    def apply_mix(self, results: dict) -> dict:
        ...
        h, w = img.shape[:2]
        bboxes = clip(bboxes, "xyxy", w, h)
        kpts, kpts_vis = clip(kpts, "keypoints_vis", max_width=w, max_height=h, keypoints_visible=kpts_vis)
    ...
    def _create_mixup_image(self, results, mixed_data_list):
        ...
        aux_img = imresize(aux_img, (int(aux_img.shape[1] * scale_ratio), int(aux_img.shape[0] * scale_ratio)))
        ...
        out_img = imresize(out_img, (int(out_img.shape[1] * jit_factor), int(out_img.shape[0] * jit_factor)))
        ...
        bboxes *= scale_ratio
        bboxes = clip(bboxes, "xyxy", aux_w, aux_h)
        if is_filp:
        ...
        annos["id"] = [results["id"], aux_results["id"]]

        # keypoints
        kpts = aux_results["keypoints"] * scale_ratio
        kpts, kpts_vis = clip(kpts, "keypoints_vis", max_width=aux_w, max_height=aux_h, keypoints_visible=aux_results["keypoints_visible"])
        if is_filp:
