...
from precision_track.registry import RUNNERS, MODELS
from precision_track.utils import CheckpointLoader, load_checkpoint_to_model, get_device, load_state_dict
...
class TrackingRunner(SingleRunner):
    ...
    def train(self) -> nn.Module:
        ...
        if self.detector_param_schedulers is not None:
            self.detector_param_schedulers = self.build_param_scheduler(  # type: ignore
                self.detector_param_schedulers
            )  # type: ignore
        ...
        if modules is not None:
            self.logger.info(f'Enabling the "activation_checkpointing" feature' f" for sub-modules: {modules}")
        ...
        if modules is not None:
            self.logger.info(f'Enabling the "efficient_conv_bn_eval" feature' f" for sub-modules: {modules}")
