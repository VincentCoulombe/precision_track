...
def resume(self, filename: str, resume_optimizer: bool = True, resume_param_scheduler: bool = True, map_location: Union[str, Callable] = "default") -> None:
    ...
    if resumed_seed is not None and resumed_seed != current_seed:
        if current_seed is not None:
            self.logger.warning(
                f"The value of random seed in the " f'checkpoint "{resumed_seed}" is ' f"different from the value in " f'`randomness` config "{current_seed}"'
            )
