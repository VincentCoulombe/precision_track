...
from .rewriter_utils import Checker, ContextCaller, RewriterRegistry, copy_function, get_frame_func, get_func_qualname, import_function
...
class FunctionRewriter:
    ...
    def register_rewriter(
        self, func_name: str, backend: str = "onnxruntime", ir: str = "onnx", extra_checkers: Optional[Union[Checker, List[Checker]]] = None, **kwargs
    ):
    ...
    def get_context(self, key: Optional[str] = None) -> ContextCaller:
        ...
        if ctx is None:
            MMLogger.get_instance("mmengine", log_level=logging.INFO, file_mode="w").warning(f"Can not found context of {key}")
        return ctx
