...
class ByteTrack(BaseAssignationAlgorithm):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """ByteTrack: Building trajectories by doing detections-tracks associations
        ...
        assert isinstance(obj_score_thrs, dict) and 0.0 <= obj_score_thrs["high"] <= 1.0 and 0.0 <= obj_score_thrs["low"] <= 1.0
        ...
        assert (
            isinstance(match_iou_thrs, dict)
            and 0.0 <= match_iou_thrs["high"] <= 1.0
            and 0.0 <= match_iou_thrs["low"] <= 1.0
            and 0.0 <= match_iou_thrs["tentative"] <= 1.0
        )
        self.match_iou_thrs = match_iou_thrs

    def update_thresholds(self, tracking_thresholds: dict) -> None:
        if "low_thr" in tracking_thresholds:
            self.obj_score_thrs["low"] = tracking_thresholds["low_thr"]
        if "init_thr" in tracking_thresholds:
            self.init_track_thr = tracking_thresholds["init_thr"]
        if "conf_thr" in tracking_thresholds:
            self.obj_score_thrs["high"] = tracking_thresholds["conf_thr"]
