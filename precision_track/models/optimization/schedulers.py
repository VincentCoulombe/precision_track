...
class QuadraticWarmupParamScheduler(_ParamScheduler):
    ...
    def __init__(
        self, optimizer: Optimizer, param_name: str, begin: int = 0, end: int = INF, last_step: int = -1, by_epoch: bool = True, verbose: bool = False
    ):
        if end >= INF:
            raise ValueError(
                "``end`` must be less than infinity," "Please set ``end`` parameter of " "``QuadraticWarmupScheduler`` as the " "number of warmup end."
            )
        self.total_iters = end - begin
        super().__init__(optimizer=optimizer, param_name=param_name, begin=begin, end=end, last_step=last_step, by_epoch=by_epoch, verbose=verbose)
