...
class CSPNeXtNeck(BaseModule):
    ...
    def __init__(
        ...
        upsample_cfg: Config = dict(scale_factor=2, mode="nearest"),
        conv_cfg: bool = None,
        norm_cfg: Config = dict(type="BN", momentum=0.03, eps=0.001),
        act_cfg: Config = dict(type="Swish"),
        init_cfg: Config = dict(type="Kaiming", layer="Conv2d", a=math.sqrt(5), distribution="uniform", mode="fan_in", nonlinearity="leaky_relu"),
        **kwargs,
    ) -> None:
        ...
        for i in range(len(in_channels)):
            self.out_convs.append(conv(in_channels[i], out_channels, 3, padding=1, conv_cfg=conv_cfg, norm_cfg=norm_cfg, act_cfg=act_cfg))
